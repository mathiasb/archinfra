...
echo ""
echo "CD pipeline setup (run as root):"
echo "  5. Install Claude CLI: npm install -g @anthropic-ai/claude-code"
echo "  6. Set up BuildKit + Claude CLI: sudo REGISTRY_CREDS='mathias:<token>' bash scripts/buildkitd-setup.sh"
echo "  7. Configure /etc/environment with API keys (see docs/cd-pipeline.md#environment-variables)"
echo "  8. Set REGISTRY_CREDS and INFRA_DEPLOY_KEY as Gitea org secrets"
echo "  9. Restore sops-age Secret to flux-system namespace (from 1Password backup):"
